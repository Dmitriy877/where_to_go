...
```
* Перейдите в консоли в папку с проектом и установите настройки переменного окружения
```
...
```
* Имеется возможность добавлять места при помощи ссылки на json файл. json файл должен быть типа 
```python
{
...
* [environs](https://pypi.org/project/environs/) - Библиотека Python для анализа переменных окружения.
